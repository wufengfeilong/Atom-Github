...
public class WaterFeeController extends BaseController {
    ...
    public Object waterFeeEdit(){
        PageData pd = this.getPageData();
        Map<String, Object> returnMap = null;
        try {
            returnMap = waterFeeService.waterFeeEdit(pd);
        } catch (Exception e) {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/waterList", produces = {"application/json;charset=UTF-8"})
    public Object waterFeeList() {
        PageData pd = this.getPageData();
        ...
        try {
            ...
            pd.put("numE",10);
            List<PageData> waterFeeList = waterFeeService.getWaterFeeList(pd);
            int count = waterFeeService.getWaterFeeListCount();
            ...
            // 设定水费列表的查询结果
            data.put("listWater", waterFeeList);
            // 设定总页数
    ...
    public  Object send() throws Exception {
        ...
        try{
            ...
            if (!toAddress.isEmpty()) {
                ...
                // 邮件主题
                message.setSubject("水费收取");
                // 邮件容器
                MimeMultipart mimeMultiPart = new MimeMultipart();
                // 设置HTML
                BodyPart bodyPart = new MimeBodyPart();
                // 邮件内容
                bodyPart.setContent(getMailContent(recordDate, fee), "text/html;charset=utf-8");
     ...
     * @return
     */
    private String getMailContent(String recordDate, String fee){
        ...
        html +="	<div><span>您截止到</span><span>" +
                recordDate + "</span>，共计<span>" +
                fee + "</span>元水费请及时缴纳。</span></div>";
