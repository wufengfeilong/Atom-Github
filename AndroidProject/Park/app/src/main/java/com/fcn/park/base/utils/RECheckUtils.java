...
public class RECheckUtils {
     ...
     * @return
     */
    public static boolean checkGroupCode(String code){
        if (code.length() == 10 || code.length() == 18) {
            String regularExpression1 = "^[0-9A-Z]{18}$";
            Pattern r1 = Pattern.compile(regularExpression1);
            Matcher m1 = r1.matcher(code);
            boolean matches1 = m1.matches();
            String regularExpression2 = "[0-9]{8}-[a-zA-Z0-9]";
            Pattern r2 = Pattern.compile(regularExpression2);
            Matcher m2 = r2.matcher(code);
