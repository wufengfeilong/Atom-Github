package sdwxwx.com.release.view;
...
public class RecordProgressController {
     ...
     * 镜头速度
     */
    private float mRecordingSpeed = 1.0f;
    ...
    private class ChangeProgressRunnable implements Runnable {
        @Override
        public void run() {
            if (getTotalRecordTime() >= Constant.MAX_DURATION) {
    ...
    public boolean isPassMinPoint() {
        ...
        return recordedTime >= Constant.MIN_DURATION;
    }
    ...
    public float getRecordingSpeed() {
        return mRecordingSpeed;
    }

    public void setRecordingSpeed(float recordingSpeed) {
        this.mRecordingSpeed = recordingSpeed;
    }
    ...
    public int getRecordedTime() {
        return mProgressView.mTotalWidth * Constant.MAX_DURATION / mProgressView
                .mScreenWidth;
    }
    ...
    private RecordProgressTimer.ProgressUpdateListener mProgressUpdateListener = new RecordProgressTimer.ProgressUpdateListener() {
        ...
        public void updateProgress(long interval) {
            if (!mProgressClipList.isEmpty()) {
                ...
                // 控制慢镜头和快镜头
                clip.timeInterval = (long) ((float)interval/mRecordingSpeed) ;
                mHandler.post(mProgressRunnable);
            }
        }
    };
