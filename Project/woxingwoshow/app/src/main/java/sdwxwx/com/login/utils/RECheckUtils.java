...
public class RECheckUtils {
    ...
    public static boolean checkIdCard(String idcard){
        ...
        // 如果符合18位身份证的规范直接返回true
        if (m.matches()) {
            return true;
        } else {
            // 18位验证失败的情况，再次验证是否符合15位身份证的规范
            regularExpression = "^[1-9]\\d{5}\\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\\d{2}$";
            r = Pattern.compile(regularExpression);
            m = r.matcher(idcard);
            return m.matches();
        }
    }
